...
export default function FactoryNav({ role = 'admin' }) {
    const navItems = [
    ...
    return (
        <HStack spacing="8px" mt="12px">
            <Button
                as={NavLink}
                to={role === 'admin' ? `/factories/${factoryId}` : `/supplier/factories/${factoryId}`}
                variant="ghost"
                size="sm"
                fontWeight="600"
                sx={isProductsActive ? {
            ...
            {navItems.map((item) => (
                <Button
                    key={item.label}
                    as={NavLink}
                    to={role === 'admin' ? `/factories/${factoryId}${item.to}` : `/supplier/factories/${factoryId}${item.to}`}
                    end={item.end}
                    variant="ghost"
                    size="sm"
                    fontWeight="600"
