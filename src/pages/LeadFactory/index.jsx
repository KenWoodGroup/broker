...
const STATUSES = Object.entries(STATUS_CONFIG).map(([value, c]) => ({
  value,
  ...c,
}));
    ...
    >
      ...
      <Flex direction="column" gap="7px" mb="14px">
        <Flex align="center" gap="8px">
          <Phone size={13} color={labelColor} style={{ flexShrink: 0 }} />
        ...
        <Flex align="flex-start" gap="8px">
          ...
          />
          <Text fontSize="12px" color={textMuted} noOfLines={2}>
            {row.address}
          </Text>
        </Flex>
        ...
        {row.director_name && (
          <Flex align="center" gap="8px">
            <User size={13} color={labelColor} style={{ flexShrink: 0 }} />
        ...
        {row.direction && (
          <Flex align="center" gap="8px">
            <Tag size={13} color={labelColor} style={{ flexShrink: 0 }} />
            <Text fontSize="12px" color={textMuted}>
              {row.direction}
            </Text>
          </Flex>
...
export default function UnverifiedFactories({ role = "admin" }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Xatolik",
        description: e?.response?.data?.message || "Yuklab bo'lmadi",
        status: "error",
        duration: 3000,
      });
    } finally {
  ...
  const handleSave = async () => {
    if (!formData.name || !formData.phone || !formData.address) {
      toast({
        title: "Majburiy maydonlarni to'ldiring",
        status: "warning",
        duration: 2500,
      });
      return;
    ...
    try {
      if (editTarget) {
        await $api.patch(`${BASE}/${editTarget.id}/status`, {
      ...
      toast({
        title: editTarget ? "Yangilandi" : "Qo'shildi",
        status: "success",
        duration: 2000,
      });
      createModal.onClose();
      ...
      toast({
        title: "Xatolik",
        description: e?.response?.data?.message || "Saqlab bo'lmadi",
        status: "error",
        duration: 3000,
      });
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      await $api.patch(`${BASE}/${rejectTarget.id}/reject`, {
        reason: rejectReason,
      });
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await $api.patch(`${BASE}/${statusTarget.id}/status`, {
        status: newStatus,
      });
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      toast({
        title: "Xatolik",
        description: e?.response?.data?.message || "Tekshirib bo'lmadi",
        status: "error",
        duration: 3000,
      });
    } finally {
  ...
  return (
    <Box px="24px" pt="24px" pb="40px" bg={pageBg} minH="100vh">
      ...
      <Flex justify="center" align="center" gap={2} mt="28px">
        ...
        <Text fontSize="13px" fontWeight="500" minW="64px" textAlign="center">
          {page} / {totalPages}
        </Text>
        <Button
          size="sm"
      ...
      <Modal isOpen={checkModal.isOpen} onClose={checkModal.onClose} size="md">
        ...
        <ModalContent borderRadius="12px">
          ...
          <ModalBody>
            ...
            {!checkData ? (
              <Flex direction="column" align="center" gap={3} py={4}>
                <Text fontSize="13px" color={textMuted} textAlign="center">
                ...
                {checkData.erp_check?.found_in_erp ? (
                  <Box>
                    <Flex align="center" gap={2} mb={3}>
                      ...
                      <Text fontSize="13px" fontWeight="700" color="green.700">
                        Bazada mavjud
                      </Text>
                    </Flex>
                  ...
                  <Flex align="center" gap={2} py={2}>
                    ...
                    <Text fontSize="13px" fontWeight="700" color="red.600">
                      Bazada topilmadi
                    </Text>
                  </Flex>
                )}
          ...
          <ModalFooter gap={2} pt={3}>
            ...
            <Button variant="ghost" size="sm" onClick={checkModal.onClose}>
              Yopish
            </Button>
          </ModalFooter>
        </ModalContent>
      ...
      >
        ...
        <ModalContent borderRadius="12px">
            ...
            <Flex direction="column" gap={3}>
              {[
                ...
                },
                {
                  name: "direction",
                  label: "Yo'nalish",
                  placeholder: "Metall, qurilish...",
                },
              ].map((f) => (
                <FormControl key={f.name}>
                  <FormLabel fontSize="13px" mb={1}>
                    {f.label}
                  </FormLabel>
                  <Input
                    name={f.name}
                    ...
                    borderRadius="8px"
                    size="sm"
                    isReadOnly={!!editTarget && f.name !== "status"}
                    opacity={editTarget && f.name !== "status" ? 0.65 : 1}
                  />
                </FormControl>
              ))}
              <FormControl>
          ...
          <ModalFooter gap={2} pt={3}>
            <Button variant="ghost" size="sm" onClick={createModal.onClose}>
              Bekor qilish
            </Button>
            <Button
              colorScheme="blue"
      ...
      >
        <ModalOverlay bg="blackAlpha.500" />
        <ModalContent borderRadius="12px">
          <ModalHeader fontSize="15px" pb={2}>
            Status yangilash
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Text fontSize="12px" color={textMuted} mb={3}>
              {statusTarget?.name}
            </Text>
            <Select
              value={newStatus}
          ...
          <ModalFooter gap={2} pt={3}>
            <Button variant="ghost" size="sm" onClick={statusModal.onClose}>
              Bekor qilish
            </Button>
            <Button
              colorScheme="blue"
      ...
      >
        ...
        <ModalContent borderRadius="12px">
          ...
          <ModalFooter gap={2} pt={3}>
            <Button variant="ghost" size="sm" onClick={rejectModal.onClose}>
              Bekor qilish
            </Button>
            <Button
              colorScheme="orange"
        ...
        <ModalContent borderRadius="12px">
          ...
          <ModalBody>
            <Text fontSize="14px" color={textMuted}>
              <b>{deleteTarget?.name}</b> — bu zavodini o'chirmoqchimisiz? Bu
              amalni qaytarib bo'lmaydi.
            </Text>
          </ModalBody>
          <ModalFooter gap={2} pt={3}>
            <Button variant="ghost" size="sm" onClick={deleteModal.onClose}>
              Bekor qilish
            </Button>
            <Button
              colorScheme="red"
