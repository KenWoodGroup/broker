import {
  ...
  Button, Spinner, Select, SimpleGrid, Badge,
...
import {
  ...
  CheckCircle, RefreshCw, MapPin, Tag, AlertCircle,
...
function LeadFactoryCard({
  ...
  return (
    ...
    >
      ...
      <VStack align="stretch" spacing="10px" pr="44px" w="100%" minW={0}>
        <Text fontWeight="600" fontSize="lg" noOfLines={2} w="100%" minW={0} wordBreak="break-word">
          {row?.name || "-"}
        </Text>
        ...
        <EntityCardDetailRows
          ...
          onAddressClick={row?.lat && row?.lng ? openMapWithCurrentLocation : undefined}
          addressInteractive={!!(row?.lat && row?.lng)}
        />

        {row.direction ? (
          <HStack spacing="8px" color={textSub} align="start" w="100%">
            <Tag size={16} style={{ flexShrink: 0, marginTop: 2 }} />
            <Text flex={1} minW={0} fontSize="sm" noOfLines={2} wordBreak="break-word">
              {row.direction}
            </Text>
          </HStack>
        ) : null}
        ...
        {row?.children?.length > 0 && (
          <Box w="100%" minW={0} pt="8px">
            ...
            <VStack align="start" spacing="2px" w="100%" minW={0}>
              {row.children.map((child) => (
                <HStack key={child.id} spacing="6px" w="100%" minW={0} align="flex-start">
...
export default function UnverifiedFactories({ role = "admin" }) {
  ...
  /* ── render ── */
  return (
    <Box pr="20px" pb="20px" pt="20px" maxW="100%" overflowX="hidden">
      ...
      <Flex
        ...
        gap={3}
      >
        <Text fontSize="xl" fontWeight="600" minW={0} flex="1 1 240px">
      ...
      <Flex gap={3} mb="14px" flexWrap="wrap" align="center">
        <Box flex={1} maxW="400px" minW="200px">
          <InputGroup size="md">
            ...
            <InputRightElement>
              {search ? (
                <IconButton size="sm" variant="ghost" aria-label="Tozalash" icon={<X size={16} />} onClick={() => setSearch("")} />
              ) : (
