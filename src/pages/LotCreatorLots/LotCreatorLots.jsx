...
export default function LotCreatorLots() {
  ...
  const CATEGORY_OPTIONS = useMemo(
    () => [
      "Umum qurilish ijtimoiy",
      "Meleratsiya va irigatsiya",
      "Avtomobil yo'lari va ko'priklar",
    ],
    [],
  );
  ...
  const fetchLots = async (next = {}) => {
    ...
    try {
      ...
      const addressValue =
        next.address !== undefined ? next.address : filters.address;
      const params = {
        type: next.type ?? filters.type ?? undefined,
        category: next.category ?? filters.category ?? undefined,
        searchName:
          (next.searchName ?? filters.searchName)?.trim() || undefined,
  ...
  const goToLotDetail = (lotId) => {
    ...
    const base = pathname.startsWith("/lotcreator")
      ? "/lotcreator/lots"
      : "/lots";
    navigate(`${base}/${lotId}`);
  };
      ...
      <Box mb="20px">
            ...
            <InputRightElement>
              {filters.searchName?.trim() ? (
                <IconButton
                  ...
                  icon={<X size={13} />}
                  onClick={() =>
                    setFilters((p) => ({ ...p, searchName: "", page: 1 }))
                  }
                />
              ) : (
        ...
        <Flex gap={3} flexWrap="wrap" align="center">
          ...
          <Select
            ...
            onChange={(e) =>
              setFilters((p) => ({ ...p, address: e.target.value, page: 1 }))
            }
        ...
        {loading ? (
          ...
          <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} spacing="20px">
            {lots.map((lot) => {
              ...
              const lotNumber =
                lot?.lot_number ?? lot?.lotNumber ?? lot?.number ?? "—";
              const amount = formatMoneyUz(
                lot?.amount ?? lot?.sum ?? lot?.total_sum,
              );
